const markdownEl = document.getElementById('markdown');
    ...
    function render() {
      ...
      statusEl.textContent = md.trim() ? `已转换（模板：${themeSelect.options[themeSelect.selectedIndex].text}）` : '';
    }
    ...
    function cleanWechatResidualStyles() {
      const html = previewEl.dataset.html || previewEl.innerHTML;
      if (!html.trim()) {
        statusEl.textContent = '没有可清理的内容，请先转换';
        return;
      }
        ...
        if (style) {
          const cleaned = style
            ...
            .filter(rule => {
              ...
              return !['position', 'left', 'right', 'top', 'bottom', 'transform', 'z-index', 'max-height', 'min-height'].includes(key);
            })
            .join(';');
      ...
      const cleanedHtml = root.outerHTML;
      previewEl.innerHTML = cleanedHtml;
      previewEl.dataset.html = cleanedHtml;
      statusEl.textContent = '已清理微信残留样式，可重新复制';
    }

    document.getElementById('convertBtn').addEventListener('click', render);
    document.getElementById('cleanBtn').addEventListener('click', cleanWechatResidualStyles);
